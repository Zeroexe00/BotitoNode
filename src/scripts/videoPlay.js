...
export const connectVoiceChannel = async (msg, str = '', client) => {
  try {
    ...
    const voice_Channel = await client.channels.fetch(msg.member.voice.channelID);
    ...
    if(str.includes('https://')) {
      const stream = ytdl(str, {
        filter: "audioonly",
        opusEncoded: true,
        encoderArgs: ['-af', 'bass=g=10,dynaudnorm=f=200']
      });
      const repro = await voice_Connection.play(stream, { type: 'opus' });
      await repro.on('finish', () => {
        msg.guild.me.voice.channel.leave();
      })
    } else if (msg.content === 'hablemos') {
      const repro = await voice_Connection.play('sound.mp3');
      await repro.on('finish', () => {
        msg.guild.me.voice.channel.leave();
      })
      ...
      const repro = await voice_Connection.play('sound/luffy_sugoi.mp3');
      await repro.on('finish', () => {
        msg.guild.me.voice.channel.leave();
      })
    }
